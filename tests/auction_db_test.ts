...
import { categories } from '../src/auction/types/enums';
...
function testAddAndRetrieveListing() {
  ...
  const listing = new AuctionListing(item, 500, new Date(), '123', 'Curtidor', categories[0]);
...
function testMultipleListingsSameSeller() {
  ...
  for (let i = 0; i < categories.length; i++) {
    const item = new ItemStack('minecraft:diamond_sword', 1);
    const listing = new AuctionListing(item, 100 + i, new Date(), 'id-' + i, seller, categories[i]);
    manager.insertListing(listing);
  }
...
function testRemoveListing() {
  ...
  const listing = new AuctionListing(item, 300, new Date(), 'del-1', 'Remover', categories[1]);
